...
**关键参数说明：**
*   `--suffix _<版本号>`: **必须**。GPUStack 根据此后缀识别多版本（例如 `vllm_0.7.3`）。
*   `--pip-args`: 可选。用于指定该环境中需要覆盖的依赖（如 `transformers`）。

### 示例：安装 vLLM v0.7.3 并搭配最新 Transformers

```bash
# 安装 vLLM 0.7.3，同时强制安装较新的 transformers 以支持 GLM-4.7 等新模型
pipx install --force \
    --suffix _0.7.3 \
...
*(注意：请根据 vLLM 官方文档选择兼容的 torch 版本。如果不指定 pip-args，pipx 会安装 vllm 默认依赖)*
...
```bash
ls -l /var/lib/gpustack/bin/vllm_0.7.3
```
...
回到 GPUStack 网页控制台：
1. 编辑或部署你的模型。
2. 在 **"Backend Version / 后端版本"** 输入框中，准确填入你刚才安装的版本号：`0.7.3`。
3. 保存/部署。

GPUStack 会检测到本地 `bin` 目录下已经存在 `vllm_0.7.3`，因此**不会**再次触发下载，而是直接使用你手动准备好的环境运行模型。
