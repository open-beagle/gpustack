# vLLM 0.16.0 + Transformers v5 快速指南

本指南介绍如何在 GPUStack 中使用 vLLM 0.16.0 配合 transformers v5 来运行需要最新 transformers 版本的模型（如 GLM-4.7 Flash）。

## 背景

- vLLM 0.16.0 开始原生支持 transformers v5
- 某些新模型（如 GLM-4.7 Flash）需要 transformers v5 才能正常运行
- 通过 `pipx` 的 `--pip-args` 参数，可以确保安装 transformers v5
...
### 2. 安装 vLLM 0.16.0 + Transformers v5
...
# 安装 vLLM 0.16.0 + transformers v5
pipx install --force \
  --suffix _v0.16.0 \
...
```bash
# 检查可执行文件是否存在
ls -l /var/lib/gpustack/bin/vllm_v0.16.0

# 检查 transformers 版本
/var/lib/gpustack/pipx/venvs/vllm-v0.16.0/bin/python -c "import transformers; print(transformers.__version__)"
```
...
2. 展开 **"Advanced"** 高级设置
3. 在 **"Backend Version"** 字段填入：`0.16.0`
...
| --------- | ----------------- | ------------------------------------------- |
| 0.16.0+   | >= 5.0.0          | GLM-4.7 Flash 等需要 transformers v5 的模型 |
...
vLLM 0.16.0 开始原生支持 transformers v5，相比 0.15.0 更稳定，兼容性更好。
...
```bash
# 方法 1：使用 pipx 卸载
docker exec -it <container> pipx uninstall vllm-v0.16.0

# 方法 2：直接删除目录
docker exec -it <container> rm -rf /var/lib/gpustack/pipx/venvs/vllm-v0.16.0
docker exec -it <container> rm -f /var/lib/gpustack/bin/vllm_v0.16.0
```
...
- [vLLM 0.16.0 Release Notes](https://github.com/vllm-project/vllm/releases/tag/v0.16.0)
