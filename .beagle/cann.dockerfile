...
RUN WHEEL_PACKAGE="$(ls /tmp/*-any.whl)" && \
  ...
  python3 -c "import os; p = '/usr/local/python3.11.14/lib/python3.11/site-packages/torchaudio/_extension/utils.py'; c = open(p, 'r').read() if os.path.exists(p) else ''; c = c.replace('torch.ops.load_library(paths[0])', 'try:\n        torch.ops.load_library(paths[0])\n    except OSError:\n        pass') if c else c; open(p, 'w').write(c) if c else None" &&\
  rm /tmp/*.whl && \
  pip3 cache purge
