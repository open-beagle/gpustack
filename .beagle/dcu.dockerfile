...
ENV POETRY_SOURCE_0_URL=https://mirrors.cloud.aliyuncs.com/pypi/simple/
...
RUN pip config set global.index-url https://mirrors.cloud.aliyuncs.com/pypi/simple/ \
    ...
    && pipx install $WHEEL_PACKAGE --pip-args="--index-url https://mirrors.cloud.aliyuncs.com/pypi/simple/ --timeout=600 --retries=5" \
    && pip cache purge
